    ...
    def _normalize_agenda_format(self, content: str) -> str:
        ...
        while i < len(lines):
            ...
            if current_section == "fixpunkte":
                ...
                # Lines with existing tags - normalize format (strip asterisks)
                if i + 1 < len(lines) and (lines[i + 1].strip().startswith("*") or lines[i + 1].strip().startswith("#")):
                    # Keep title, skip old tag lines
                    result.append(line)
                    i += 1
                    # Collect tags from existing lines
                    collected_tags: list[str] = []
                    while i < len(lines):
                        tl = lines[i].strip()
                        if tl.startswith("*") and tl.endswith("*"):
                            # Strip asterisks and collect
                            collected_tags.extend(tl.strip("*").strip().split())
                            i += 1
                        elif tl.startswith("#"):
                            collected_tags.append(tl)
                            i += 1
                        else:
                            break
                    # Output normalized tags without asterisks
                    if collected_tags:
                        result.append(" ".join(collected_tags))
                    result.append("")
                    continue
