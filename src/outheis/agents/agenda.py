...
class AgendaAgent(BaseAgent):
    ...
    def run_review(self, force: bool = False) -> None:
        ...
        query = (
            ...
            "     Hard limit: Today holds a MAXIMUM of 5 items total.\n"
            "     Mandatory (always add, regardless of count — these can push Today past 5 if unavoidable):\n"
            ...
            "     Optional fill (only if Today has fewer than 5 items after mandatory pass):\n"
            "       Fill in chronological order up to 5 total, sourcing from items up to 30 days out:\n"
            "       - #date within 30 days → candidate.\n"
            "       - #action-required with a future date → candidate.\n"
            ...
            "   (e.g. 'Rückmeldung Förderantrag' in Shadow and 'Förderantrag abschicken' in Today may be the same).\n"
            "   Present only ONE consolidated entry in Agenda.md — the most complete or actionable formulation.\n"
            "   BACKPROPAGATION (prevent 'Wiedergänger'):\n"
            "   - When an item is marked done or consolidated: update Shadow.md (prepend #done-YYYY-MM-DD) AND\n"
            "     ask_zeno to update the source vault file — same as rule 7 completion workflow.\n"
            "   - When two entries are merged: keep the surviving entry, mark the discarded one #done-YYYY-MM-DD\n"
            "     in Shadow.md and in vault. Without backpropagation, shadow_scan will restore the discarded item.\n"
            "   SCOPE: vault files, chat-sourced items, Exchange.md entries, Shadow.md items — all four.\n"
            "\n"
