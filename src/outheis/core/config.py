...
def get_tasks_path() -> Path:
    """Path to tasks.json — persisted task registry, readable without HTTP."""
    return get_human_dir() / "tasks.json"
        ...
        if self.provider_aliases:
            ...
            for provider in order:
                if skip_providers and provider in skip_providers:
                    continue
                model_name = self.provider_aliases.get(provider, {}).get(alias)
                if model_name:
                    warning = None
                    if skip_providers:
                        warning = (
...
def init_directories() -> None:
    ...
    try:
        config = load_config()
        for vault in config.human.all_vaults():
            (vault / "Agenda").mkdir(parents=True, exist_ok=True)
    except Exception:
        pass
