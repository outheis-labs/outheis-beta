...
class TestConfigExampleParseable:
    ...
    def test_agents(self, config_from_example):
        ...
        assert agents.get("alan") is not None
        assert agents["alan"].enabled is False  # alan disabled by default (dev only)

    def test_schedule(self, config_from_example):
        sched = config_from_example.schedule
        assert sched.agenda_review.hourly_at_minute == 55
        assert sched.agenda_review.start_hour == 4
        assert sched.agenda_review.end_hour == 23
        assert sched.pattern_nightly.hour == 4
        assert sched.index_rebuild.hour == 4
