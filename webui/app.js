...
function waitForEditor(timeout = 8000) {
  if (window.MarkdownEditor) return Promise.resolve(window.MarkdownEditor);
  return new Promise((resolve) => {
    const deadline = Date.now() + timeout;
    const poll = setInterval(() => {
      if (window.MarkdownEditor) { clearInterval(poll); resolve(window.MarkdownEditor); }
...
async function loadFile(type, filename) {
  ...
  if (editor) {
    currentEditor = editor.create(body, content, { onSave: saveCurrentFile });
  } else {
...
async function saveCurrentFile() {
  const content = currentEditor ? currentEditor.getContent() : '';
